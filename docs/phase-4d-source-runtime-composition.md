...
Phase 4D introduces a single server-side runtime evidence architecture. It
...
Phase 4D implements one deliberately narrow deterministic detector:
...
list. The source inspector states that no answer was invented. Phase 4D does
not persist an information-gap record and does not ask an LLM to fill the gap.
...
members and actors from another tenant are denied. Unknown source-class input,
overlong queries, and out-of-range limits are rejected before retrieval.
...
and secrets. Phase 4D does not persist every inspector page load. A future
...
Phase 4D establishes the source labels, adapter registry, safe evidence shape,
authorization boundary, provenance, conflicts, unknown state, and audit
