...
- Every consumed OOXML part (`[Content_Types].xml`, `xl/workbook.xml`, `xl/_rels/workbook.xml.rels`, `xl/sharedStrings.xml`, and each worksheet) is run through a bounded structural XML parser before any business-data extraction. The parser requires one well-formed document with the expected root (`Types`, `workbook`, `Relationships`, `sst`, or `worksheet`), matching start/end tags, and no trailing sibling root or non-whitespace trailing content. Namespace prefixes on those roots are accepted. Rootless fragments, misrooted wrappers, multiple roots, malformed nesting, DTDs/entities, and unterminated constructs fail closed as `malformed`.
...
- XLSX reuses the existing `jszip` dependency already required for DOCX ZIP inspection, with the same class of entry/expansion/ratio/nesting/time guards. A bounded in-repo XML scanner then verifies well-formedness and the expected document root for each consumed part (depth, name, and attribute limits; no DOM library). Comments and processing instructions are omitted; CDATA payloads are reinserted as escaped character data so later tag scans cannot see non-element markup. Markup is then read only from inside that validated root. There is no general XML tree, DTD/entity expansion, or formula evaluation.
