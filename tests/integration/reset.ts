import { PrismaClient } from "@prisma/client";

/**
 * Wipe Phase 1–5A tables in FK-safe order for integration tests.
 * Child tables are deleted before Organization.
 */
export async function resetApplicationData(
  prisma: PrismaClient,
): Promise<void> {
  await prisma.prospectContactCustomValue.deleteMany();
  await prisma.prospectCustomValue.deleteMany();
  await prisma.prospectChannel.deleteMany();
  await prisma.prospectMerge.deleteMany();
  await prisma.prospectContact.deleteMany();
  await prisma.prospect.deleteMany({ where: { lifecycle: "MERGED" } });
  await prisma.prospect.deleteMany();

  await prisma.user.updateMany({ data: { activeOrganizationId: null } });
  await prisma.organizationAuditEvent.deleteMany();

  await prisma.csvImportKnowledgeRowActivation.deleteMany();
  await prisma.csvImportOfferingRowActivation.deleteMany();
  await prisma.csvImportConfirmation.deleteMany();
  await prisma.csvImportRow.deleteMany();
  await prisma.csvImport.deleteMany();

  // Phase 4C (before custom-field definitions and organization delete)
  await prisma.offeringVariantPrice.deleteMany();
  await prisma.offeringCustomValue.deleteMany();
  await prisma.offeringEligibility.deleteMany();
  await prisma.offeringVariant.deleteMany();
  await prisma.offeringFeature.deleteMany();
  await prisma.offeringPrice.deleteMany();
  await prisma.offeringVersion.deleteMany();
  await prisma.offering.deleteMany();

  // Phase 4B (before organization delete)
  await prisma.knowledgeDocumentIssue.deleteMany();
  await prisma.knowledgeDocumentJob.deleteMany();
  await prisma.knowledgeDocument.deleteMany();

  // Phase 4A (before organization delete)
  await prisma.knowledgePassage.deleteMany();
  await prisma.knowledgeSection.deleteMany();
  await prisma.knowledgeVersion.deleteMany();
  await prisma.knowledgeSource.deleteMany();

  // Phase 3B (before organization delete)
  await prisma.organizationConfigProgress.deleteMany();
  await prisma.organizationNotificationDefaults.deleteMany();
  await prisma.organizationRecordingConsentPolicy.deleteMany();
  await prisma.organizationCallbackPolicy.deleteMany();
  await prisma.callDispositionDefault.deleteMany();
  await prisma.leadStageDefault.deleteMany();
  await prisma.holidayClosure.deleteMany();
  await prisma.serviceArea.deleteMany();
  await prisma.customFieldDefinition.deleteMany();
  await prisma.organizationLocaleSettings.deleteMany();
  await prisma.organizationTemplateAssignment.deleteMany();

  // Phase 3A
  await prisma.organizationOnboarding.deleteMany();
  await prisma.organizationSettings.deleteMany();
  await prisma.operatingHourInterval.deleteMany();
  await prisma.businessService.deleteMany();
  await prisma.businessProduct.deleteMany();
  await prisma.businessLocation.deleteMany();
  await prisma.businessProfile.deleteMany();

  // Phase 2
  await prisma.organizationInvitation.deleteMany();
  await prisma.membership.deleteMany();
  await prisma.organization.deleteMany();

  // Phase 1
  await prisma.authToken.deleteMany();
  await prisma.rateLimitBucket.deleteMany();
  await prisma.user.deleteMany();
}
