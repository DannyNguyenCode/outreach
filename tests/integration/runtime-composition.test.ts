...
describe("Phase 4D runtime composition", () => {
  ...
  it("denies cross-tenant, inactive, and non-member composition", async () => {
    ...
    const inactive = await composeRuntimeContext({
      actor: member,
      organizationId: tenantA.organizationId,
    });
    expect(inactive.ok).toBe(false);
    if (!inactive.ok) expect(inactive.reason).toBe("inactive_membership");
  });
