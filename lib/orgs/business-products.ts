...
export async function listBusinessProducts(input: {
  ...
  try {
    await assertCanReadProducts(input);
...
export async function updateBusinessProduct(input: {
  actor: SafeUser;
  organizationId: string;
  productId: string;
  raw: unknown;
...
export async function deactivateBusinessProduct(input: {
  actor: SafeUser;
  organizationId: string;
  productId: string;
}): Promise<ProductResult> {
...
export async function reorderBusinessProducts(input: {
  actor: SafeUser;
  organizationId: string;
  raw: unknown;
}): Promise<ProductsResult> {
