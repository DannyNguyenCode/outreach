...
export async function parseXlsxWorkbook(
  ...
  enforceTabularDeadline(started);
  const safeContentTypes = parseOoxmlDocument(
    contentTypes,
    "Types",
    started,
  ).normalizedXml;
  const safeWorkbook = parseOoxmlDocument(
    workbook,
    "workbook",
    started,
  ).normalizedXml;
  const safeRels = parseOoxmlDocument(
    rels,
    "Relationships",
    started,
  ).normalizedXml;
  ...
  for (const sheetRef of sheetRefs) {
    ...
    const parsed = parseWorksheet(
      parseOoxmlDocument(sheetXml, "worksheet", started).normalizedXml,
      info,
      sharedStrings,
      started,
    );
...
function parseWorkbookRelationships(relsXml: string): Map<string, string> {
  ...
  const inner = extractElementInner(relsXml, "Relationships");
  if (inner === null) {
    throw invalid(
      "malformed",
...
async function readSharedStrings(
  ...
  enforceTabularDeadline(started);
  const safeXml = parseOoxmlDocument(xml, "sst", started).normalizedXml;
  const values: string[] = [];
...
function hasCanonicalWorkbookMainType(contentTypes: string): boolean {
  const inner = extractElementInner(contentTypes, "Types");
  if (inner === null) {
    return false;
  }
