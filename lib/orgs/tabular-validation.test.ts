...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  ...
  it("requires one well-formed expected root on every consumed OOXML part", async () => {
    const sheets = [{ name: "Sheet1", rows: [["Name"], ["Widget"]] }];
    ...
    await expect(
      validateTabularImport({
        bytes: await makeXlsx({
          sheets,
          extras: {
            "xl/workbook.xml": `<sheets><sheet name="Sheet1" sheetId="1" r:id="rId1"/></sheets>`,
          },
        }),
        filename: "rootless-workbook.xlsx",
        declaredMimeType: XLSX_MIME,
      }),
    ).rejects.toMatchObject({ code: "malformed" });
    ...
    await expect(
      validateTabularImport({
        bytes: await makeXlsx({
          sheets,
          extras: {
            "xl/_rels/workbook.xml.rels": `<Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet" Target="worksheets/sheet1.xml"/>`,
          },
        }),
        filename: "rootless-rels.xlsx",
        declaredMimeType: XLSX_MIME,
      }),
    ).rejects.toMatchObject({ code: "malformed" });
    ...
    await expect(
      validateTabularImport({
        bytes: await makeXlsx({
          sheets,
          extras: {
            "xl/sharedStrings.xml": `<si><t>Name</t></si><si><t>Widget</t></si>`,
          },
        }),
        filename: "rootless-sst.xlsx",
        declaredMimeType: XLSX_MIME,
      }),
    ).rejects.toMatchObject({ code: "malformed" });
...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  <sheetData>
    <row r="1"><c r="A1" t="inlineStr"><is><t>Other</t></is></c></row>
        ...
        filename: "multi-root-sheet.xlsx",
        declaredMimeType: XLSX_MIME,
      }),
    ).rejects.toMatchObject({ code: "malformed" });
...
<sheets><sheet name="Evil" sheetId="9" r:id="rId9"/></sheets>`,
          },
        }),
        filename: "trailing-workbook.xlsx",
        declaredMimeType: XLSX_MIME,
      }),
    ).rejects.toMatchObject({ code: "malformed" });
...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  <sheetData>
    <row r="1"><c r="A1" t="inlineStr"><is><t>Name</t></is></c></row>
        ...
        filename: "nested-malformed-sheet.xlsx",
        declaredMimeType: XLSX_MIME,
      }),
    ).rejects.toMatchObject({ code: "malformed" });
...
<s:sst xmlns:s="http://schemas.openxmlformats.org/spreadsheetml/2006/main" count="0" uniqueCount="0"></s:sst>`,
          "xl/worksheets/sheet1.xml": `<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<x:worksheet xmlns:x="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  <x:sheetData>
    <x:row r="1"><x:c r="A1" t="inlineStr"><x:is><x:t>Name</x:t></x:is></x:c></x:row>
    <x:row r="2"><x:c r="A2" t="inlineStr"><x:is><x:t>Widget</x:t></x:is></x:c></x:row>
  </x:sheetData>
</x:worksheet>`,
        },
      }),
      filename: "prefixed-roots.xlsx",
      declaredMimeType: XLSX_MIME,
    });
    expect(prefixed.outcome).toBe("ready");
    expect(prefixed.sheets.map((sheet) => sheet.name)).toEqual(["Sheet1"]);
    expect(prefixed.previewRows).toEqual([
      { sourceRowNumber: 2, cells: ["Widget"] },
    ]);
  });
